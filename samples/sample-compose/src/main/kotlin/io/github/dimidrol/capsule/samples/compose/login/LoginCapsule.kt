...
class LoginCapsule(
    ...
    override fun reduceResult(
        ...
        LoginResult.LoginSuccess -> Decision.effect(
            state = state.copy(loginOperation = OperationState.Success(Unit)),
            effect = LoginEffect.NavigateToHome
        )

        is LoginResult.LoginFailed -> Decision.effect(
            state = state.copy(loginOperation = OperationState.Failed(result.throwable)),
            effect = LoginEffect.ShowMessage(result.throwable.message ?: "Login failed")
        )
    }
    ...
    private fun submit(state: LoginState): Decision<LoginState, LoginOperation, LoginEffect> {
        if (state.email.isBlank() || state.password.isBlank()) {
            return Decision.effect(
                state = state,
                effect = LoginEffect.ShowMessage("Email and password must not be empty")
            )
        }

        return Decision.operation(
            state = state.copy(loginOperation = OperationState.Running),
            operation = LoginOperation.SubmitLogin(
                email = state.email,
